"""Command-line argument builders for Fast Poisson Image Editing."""
...
def get_args(gen_type: str) -> argparse.Namespace:
    """Parse arguments for the requested frontend."""
    ...
    parser.add_argument(
        "--check-backend",
        action="store_true",
        help="print all available backends",
    )
    ...
    parser.add_argument(
        "-s", "--source", type=str, help="source image filename"
    )
    if gen_type == "cli":
        parser.add_argument(
            "-m",
    ...
    parser.add_argument(
        "-t", "--target", type=str, help="target image filename"
    )
    parser.add_argument(
        "-o", "--output", type=str, help="output image filename"
    )
    if gen_type == "cli":
        parser.add_argument(
            "-h0",
            type=int,
            help="mask position (height) on source image",
            default=0,
        )
        parser.add_argument(
            "-w0",
            type=int,
            help="mask position (width) on source image",
            default=0,
        )
        parser.add_argument(
            "-h1",
            type=int,
            help="mask position (height) on target image",
            default=0,
        )
        parser.add_argument(
            "-w1",
            type=int,
            help="mask position (width) on target image",
            default=0,
        )
    parser.add_argument(
        "-g",
